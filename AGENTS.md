...
> 更新：2026-08-01 · v2.0.0 安全内核 + v4.10.0 科学战役程序
...
v4.10.0 已发布：假设战役框架 + 两个制度设计战役（诚实负性结果）+ 五年
长期演进研究 + 赛季边界协议。
...
| v4.10.0 科学战役 | 战役框架 6/6 + 长期研究 3/3 + 赛季协议 5/5；两战役 + 五年研究已发布 |
| v4.10.1 战役收尾 | 底线网格 3/5 + 多赛季研究 3/3；全部路标实现 |
...
- Trust 2 车道（本地复现 verified + 90 天 pending）；DeepSeek 调用/Token/
  ...
  保留但休眠、不再呈现。
- 战役 v1 2/5（互助不提升 RALR、安全仪器饱和）；v2 4/5（吞吐链成立、
  RALR 稳定、互助提高撤回率被拒）；v3 底线网格 3/5（撤回效应不复制、
  底线单独使 RALR +1.7pp）；五年研究通过（RALR 0.7648，零长期未决）；
  多赛季研究通过（档案链连续，RALR 0.7217）。
- `npm run campaign:run -- <id>` 与 `npm run study:long-horizon` 为本地
  科学门禁（约 13/12 分钟），不入 CI；负性结果照常发布。
