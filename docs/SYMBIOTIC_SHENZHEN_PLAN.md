...
v4.9 同屏展示两条自主可信证据：本地精确复现和 90 天墙钟生产时钟。每条
只能是 verified、pending、failed 或 stale；没有加权总分。本地结果、参考
Provider 和加速 Turn 均不能填补墙钟门禁。v4.9 按宪法决定移除外部门证要求：
签名回执、异机恢复和 live provider 不再作为门禁；其机制保留但休眠。
...
- v4.9 两条自主证据必须分别可见；90 天未到保持 pending，复现包篡改必须
  失败。外部 attestation 不作为要求。

本地复现与墙钟时长是彼此独立的证据，必须分别标注，不能互相推断。

## 后续停止条件与交接

v4.6 完成后默认不继续增加居民规模、社会机制或地理范围。后续优先级是科学
程序：预注册假设战役、长期演进研究、赛季边界协议与测试稳定性。任何新机制
仍只面向软件居民。

v4.9.0 后的开发路线：假设战役框架（假设×种子×制度×时长、held-out、exact
replay、对比报告与便携 bundle）、1825-Turn 长期研究、赛季轮转协议；90 天
墙钟车道自动推进。新面板与本地证明不能替代这些科学证据。由人类宪法参与者
在证据充分后决定是否开启下一城市机制版本。
