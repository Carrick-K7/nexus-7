...
> 更新：2026-07-19
>
> 状态：v4.0 本地 reference 实现完成；external evidence pending

## 目标与研究边界

NEXUS-7 v4 运行一个长期、可重放、可审计的合成深圳，研究具有不同物质需求的
合成人类、软件 AI 和具身机器人，能否形成可拒绝、可退出、有结果和反思的互惠
协作。全部 260 名居民都是自主软件，不含真人席位、PII、私人日记或现实地址。

这不是深圳数字孪生。公开数据只校准城市量级和行政拓扑，不能将结果表述为真实
人类行为、现实政策效果、AI 意识或法律人格证据。未来真人研究必须成为新的、
经批准的方向，不能从当前版本自动扩张。
...
- MapLibre 本地空底图展示三个合成社区及数据驱动状态；
- City Pulse 展示 Turn、人口、RALR、trace、关系、承诺、安全和模型成本；
- Community Projection 以可访问表格展示需求与资源压力；
- Event River 展示带 cursor、因果类型和双语摘要的世界事件；
- Multi-season Controls 同屏比较三个制度；
- JSON 导出绑定当前快照、报告、研究结果和事件。

WebGL 地图不是唯一信息通道。所有指标都有键盘可达表格/文本投影，并遵循减少
动画偏好、移动布局和 WCAG A/AA。
...
本地门禁通过仍只能写 `implementation complete / external evidence pending`。
远端 commit、CI/Sigstore、真实 PostgreSQL 恢复和 live provider 分别提供独立证据。

## 后续停止条件

v4.0 完成后默认不继续增加居民规模或地理范围。后续优先级依次为运行稳定性、
第二 provider 影子评估、数据保留和外部复现。只有人类明确重开真人研究方向并
批准宪法、隐私、退出和伦理流程后，才允许规划真实参与。
