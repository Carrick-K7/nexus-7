...
Clean-revision detection ignores only the exact outputs regenerated by the
release commands: iteration manifests, model/stress/readiness reports, v4.5
and v4.6 verification reports, both deterministic study projections, the v4.7
portable bundle and the manifest itself. Source edits, documentation,
configuration, renames and every unknown untracked path still set
`source.dirty=true`; no directory glob is trusted.
