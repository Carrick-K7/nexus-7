...
| Candidate code reaches receipt-signing key | receipt bridge executes only the default-branch verifier; candidate SHA is Sigstore-bound data; pull-request runs are ineligible | rotate key, invalidate receipts and audit workflow dispatches |
| Evidence allowlist drift or status downgrade | checked explicit signer override; bad-receipt precedence over missing companion files; deployed revision plus bilingual reason/code projection | reject the receipt, restore the complete allowlist and rerun the external workflow |
